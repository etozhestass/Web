...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        Method methodBefore = getMethod(pageClass, "before");
        Method methodAfter = getMethod(pageClass, "after");
        Object page;
        try {
    ...
    private static Method getMethod(Class<?> pageClass, String method) throws NotFoundException {
        Method methodBefore = null;
        for (Class<?> clazz = pageClass; methodBefore == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                methodBefore = clazz.getMethod(method, HttpServletRequest.class, Map.class);
            } catch (NoSuchMethodException ignored) {
                // No operations.
            }
        }
